pipeline {
    ...
    stages{
        stage('git cloned'){
            steps{
                git url:'https://github.com/AshishNikam123/php-project/', branch: "master"
            ...
            steps {
               script {
                   def dockerrm = 'sudo docker rm -f My-first-containe221 || true'
                    def dockerCmd = 'sudo docker run -itd --name My-first-containe221 -p 8082:80 ashishnikam/2febimg:v1'
                    sshagent(['sshkeypair']) {
                        ...
                        // sh "docker run -itd --name My-first-containe211 -p 8082:80 akshu20791/2febimg:v1"
                         ...
                         sh "ssh -o StrictHostKeyChecking=no ubuntu@172.31.4.57 ${dockerCmd}"
                    }
                }
            }
        }
